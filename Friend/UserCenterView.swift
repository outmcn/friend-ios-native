...
struct UserCenterView: View {
    ...
    private let categoryTitles = ["动态", "收藏", "点赞"]
    private var visibleItems: [BlogPageResponse] {
        switch selectedDynamicsCategory {
        case 1: return dynamics.items.filter { $0.favorited == true }
        case 2: return dynamics.items.filter { $0.thumbsUp == true }
        default: return dynamics.items
        }
    }
    ...
    var body: some View {
                            ...
                            LazyVStack(spacing: 12, pinnedViews: [.sectionHeaders]) {
                                ...
                                Section {
                                    if dynamics.items.isEmpty {
                                        ...
                                        ForEach(visibleItems) { blog in blogCard(blog, w) }
                                    }
                                } header: {
        ...
        }.navigationTitle("").navigationBarHidden(true).sheet(isPresented: $showLogin) { NavigationView { LoginView() } }.sheet(isPresented: $showFollowFans) { FollowFansView(initialTab: followFansTab) }.sheet(isPresented: $showAvatarActions) { if let info = model.userInfo { AvatarActionSheet(info: info) } }.task { cachedCity = LocationRefreshService.shared.cachedCity; await loadAll() }.onChange(of: selectedDynamicsCategory) { _ in Task { await loadSelectedCategory() } }.onReceive(NotificationCenter.default.publisher(for: .profileUpdated)) { note in if let updated = note.object as? ProfileUpdate, var current = model.userInfo { current = PersonalCenter(id: current.id, headPortrait: updated.headPortrait, nickName: updated.nickName, gender: current.gender, goldBalance: current.goldBalance, city: current.city, blogCount: current.blogCount, followCount: current.followCount, fansCount: current.fansCount, likeCount: current.likeCount, onlineDays: current.onlineDays, blog: current.blog); model.userInfo = current; if let data = try? JSONEncoder().encode(current) { UserDefaults.standard.set(data, forKey: "friend.user.center.cache") } } }.onReceive(NotificationCenter.default.publisher(for: .locationCityUpdated)) { note in cachedCity = note.object as? String }
    }
    private func applyProfileUpdate(_ updated: ProfileUpdate) { guard let current = model.userInfo else { return }; let next = PersonalCenter(id: current.id, headPortrait: updated.headPortrait, nickName: updated.nickName, gender: current.gender, goldBalance: current.goldBalance, city: current.city, blogCount: current.blogCount, followCount: current.followCount, fansCount: current.fansCount, likeCount: current.likeCount, onlineDays: current.onlineDays, blog: current.blog); model.userInfo = next; if let data = try? JSONEncoder().encode(next) { UserDefaults.standard.set(data, forKey: "friend.user.center.cache") } }
    private func loadAll() async { await model.loadIfNeeded(); dynamics.loadIfNeeded(); await loadSelectedCategory() }
    private func loadSelectedCategory() async { do { try await dynamics.load(category: selectedDynamicsCategory) } catch { dynamics.errorMessage = error.localizedDescription } }
    private func refreshAll() async { await model.refresh(); await loadSelectedCategory() }
        ...
        HStack(spacing: px(24, w)) {
            VStack(alignment: .leading, spacing: px(18, w)) {
                ...
                HStack(spacing: px(32, w)) {
                    ...
                    statValue("\(info.onlineDays ?? 0)", "天", w)
                }
            }
            Spacer(minLength: 0)
    ...
    private func dynamicList(width w: CGFloat) -> some View { VStack(alignment: .leading, spacing: 12) { dynamicsCategories(width: w); if visibleItems.isEmpty { if let error=dynamics.errorMessage { Text(error).font(.system(size: px(24, w))).foregroundStyle(.red) } else { Text("暂无动态").font(.system(size: px(24, w))).foregroundStyle(.white.opacity(0.6)) } } else { ForEach(visibleItems) { blogCard($0, w) } } }.padding(.top, px(54, w)).padding(.bottom, px(50, w)) }
    private func dynamicsCategories(width w: CGFloat) -> some View { HStack(spacing: 0) { ForEach(categoryTitles.indices, id: \.self) { i in Button { selectedDynamicsCategory = i } label: { Text(categoryTitles[i]).font(.system(size: px(24, w), weight: i == selectedDynamicsCategory ? .bold : .regular)).foregroundStyle(i == selectedDynamicsCategory ? .white : .white.opacity(0.55)).frame(maxWidth: .infinity).padding(.vertical, px(18, w)).overlay(alignment: .bottom) { if i == selectedDynamicsCategory { Color(red: 0.95, green: 0.80, blue: 0.38).frame(height: 2) } } } } }.padding(.horizontal, px(10, w)).background(Color.black.opacity(0.18)).clipShape(RoundedRectangle(cornerRadius: px(12, w))).overlay(RoundedRectangle(cornerRadius: px(12, w)).stroke(Color.white.opacity(0.12), lineWidth: 1)) }
    private func blogCard(_ blog: BlogPageResponse, _ w: CGFloat) -> some View { NavigationLink { DynamicDetailView(blog: blog) } label: { VStack(alignment: .leading, spacing: 10) { HStack { Text(relativeTime(blog.pushTime)).font(.caption).foregroundStyle(.white.opacity(0.6)); Spacer() }; if let content = blog.content, !content.isEmpty { Text(content).font(.system(size: 16)).foregroundStyle(.white) }; if let images = blog.images { ForEach(images, id: \.self) { image in DynamicImageView(urlString: image) } }; HStack { Label("\(blog.fabulous ?? 0)", systemImage: "heart"); Label("\(blog.comment ?? 0)", systemImage: "message"); if let d=blog.distanceKm { Text(String(format:"%.1fkm",d)) } }.font(.footnote).foregroundStyle(.white.opacity(0.75)) }.padding(14).frame(maxWidth: .infinity, alignment: .leading).background(Color.white.opacity(0.12)).clipShape(RoundedRectangle(cornerRadius: 16)) }.buttonStyle(.plain) }
    private func relativeTime(_ value: String?) -> String { guard let value, let date=Self.dateFormatter.date(from:value) else { return value ?? "" }; let s=Int(Date().timeIntervalSince(date)); if s < 60 { return "刚刚" }; if s < 3600 { return "\(s/60)分钟前" }; if s < 86400 { return "\(s/3600)小时前" }; return "\(s/86400)天前" }
    private static let dateFormatter: DateFormatter = { let f=DateFormatter(); f.dateFormat="yyyy-MM-dd HH:mm"; f.locale=Locale(identifier:"zh_CN"); return f }()
}
...
@MainActor final class UserCenterDynamicsViewModel: ObservableObject {
    ...
    func load(category: Int = 0) async throws { let path = category == 1 ? "community/frblog/favorite/blog/page" : category == 2 ? "community/frblog/liked/blog/page" : "community/frblog/mine/blog/page"; let r: APIEnvelope<PageResult<BlogPageResponse>> = try await APIClient.shared.request(path: path, method: "GET", body: UserCenterPageQuery(pageIndex: 1, pageSize: 100)); guard r.code == 200, let rows = r.data?.rows else { throw APIError(statusCode: r.code, message: r.msg ?? "动态数据为空") }; items = rows; if category == 0 { UserDefaults.standard.removeObject(forKey: cacheKey); if let data = try? JSONEncoder().encode(rows) { UserDefaults.standard.set(data, forKey: cacheKey) } }; errorMessage = nil }
    func toggleLike(_ item: BlogPageResponse) async throws { let r: APIEnvelope<EmptyResponse> = try await APIClient.shared.request(path: "community/frblog/like/\(item.id)", method: "POST", body: EmptyBody()); guard r.code == 200 else { throw APIError(statusCode: r.code, message: r.msg ?? "点赞失败") }; try await load(category: 0) }
}
...
struct PersonalCenter: Codable { let id: Int?; let headPortrait: String?; let nickName: String?; let gender: String?; let goldBalance: Int?; let city: String?; let blogCount: Int?; let followCount: Int?; let fansCount: Int?; let likeCount: Int?; let onlineDays: Int64?; let blog: [PersonalCenterBlog]?; var genderText: String { gender == "WOMAN" || gender == "女" ? "♀" : gender == "MAN" || gender == "男" ? "♂" : "" } }
