...
struct UserCenterView: View {
    ...
    private func topBar(width w: CGFloat) -> some View { HStack { Spacer(); HStack(spacing: px(16, w)) { Button { showFootsteps = true } label: { Image(systemName: "figure.walk").font(.system(size: px(34, w), weight: .medium)).foregroundStyle(.white) }.frame(width: px(46, w), height: px(48, w)); Button { showScanner = true } label: { Image(systemName: "qrcode.viewfinder").font(.system(size: px(33, w), weight: .medium)).foregroundStyle(.white) }.frame(width: px(46, w), height: px(48, w)); Button { showUserMenu = true } label: { ProfileActionIcon(kind: .menu).frame(width: px(42, w), height: px(42, w)) }.frame(width: px(46, w), height: px(48, w)) }.frame(width: px(190, w), alignment: .trailing).padding(.trailing, px(16, w)) }.frame(height: px(52, w)) }
