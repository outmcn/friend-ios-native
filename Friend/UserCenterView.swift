...
struct UserCenterView: View {
    ...
    private func profile(_ info:PersonalCenter,width w:CGFloat)->some View { HStack(spacing:px(46,w)){RemoteAvatar(urlString:info.headPortrait,size:px(176,w));VStack(alignment:.leading,spacing:0){Text(info.nickName ?? "用户").font(.system(size:px(44,w),weight:.bold)).foregroundStyle(.white);HStack(spacing:px(26,w)){HStack(spacing:4){Image("FriendMoneyIcon").resizable().frame(width:px(20,w),height:px(20,w));Text("余额 \(info.goldBalance ?? 0)").font(.system(size:px(24,w))).foregroundStyle(.white)};Text("IP \(info.city ?? "未知")").font(.system(size:px(28,w))).foregroundStyle(Color(red:.27,green:.28,blue:.36))}.padding(.top,px(24,w))};Spacer()} }
    ...
    private func statistic(_ v:String,_ t:String,_ w:CGFloat)->some View{VStack(spacing:4){Text(v).font(.system(size:px(36,w),weight:.bold)).foregroundStyle(.white);Text(t).font(.system(size:px(32,w))).foregroundStyle(Color(red:.27,green:.28,blue:.36))}.frame(maxWidth:.infinity)}
    ...
    private func row(_ icon:String,_ title:String,_ w:CGFloat)->some View{HStack{Image(icon).resizable().frame(width:px(46,w),height:px(46,w));Text(title).font(.system(size:px(28,w))).foregroundStyle(.white);Spacer();Image(systemName:"chevron.right").foregroundStyle(.white.opacity(.7))}.padding(.vertical,px(15,w))}
}
