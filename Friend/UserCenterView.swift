...
struct UserCenterView: View {
    ...
    private func dynamicList(width w: CGFloat) -> some View { VStack(alignment: .leading, spacing: 12) { Text("我的动态").font(.system(size: px(32, w), weight: .bold)).foregroundStyle(.white); dynamicsCategories(width: w); if dynamics.items.isEmpty { if let error=dynamics.errorMessage { Text(error).font(.system(size: px(24, w))).foregroundStyle(.red) } else { Text("暂无动态").font(.system(size: px(24, w))).foregroundStyle(.white.opacity(0.6)) } } else { ForEach(dynamics.items) { blogCard($0, w) } } }.padding(.top, px(54, w)).padding(.bottom, px(50, w)) }
    private func dynamicsCategories(width w: CGFloat) -> some View { HStack(spacing: 0) { ForEach(["动态", "锁定", "收藏", "点赞", "分类"].indices, id: \.self) { i in Button { selectedDynamicsCategory = i } label: { Text(["动态", "锁定", "收藏", "点赞", "分类"][i]).font(.system(size: px(24, w), weight: i == selectedDynamicsCategory ? .bold : .regular)).foregroundStyle(i == selectedDynamicsCategory ? .white : .white.opacity(0.55)).frame(maxWidth: .infinity).padding(.vertical, px(18, w)).overlay(alignment: .bottom) { if i == selectedDynamicsCategory { Color(red: 0.95, green: 0.80, blue: 0.38).frame(height: 2) } } } } }.background(Color.black.opacity(0.18)).clipShape(RoundedRectangle(cornerRadius: px(12, w))) }
