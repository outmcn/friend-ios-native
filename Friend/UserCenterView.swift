...
struct UserCenterView: View {
    ...
    private func topBar(width w: CGFloat) -> some View { HStack { Spacer(); HStack(spacing: px(12, w)) { Button { showFootsteps = true } label: { Image(systemName: "figure.walk").font(.system(size: px(28, w), weight: .medium)).foregroundStyle(.white) }.frame(width: px(36, w), height: px(42, w)); Button { showScanner = true } label: { Image(systemName: "qrcode.viewfinder").font(.system(size: px(27, w), weight: .medium)).foregroundStyle(.white) }.frame(width: px(36, w), height: px(42, w)); Button { showUserMenu = true } label: { ProfileActionIcon(kind: .menu).frame(width: px(34, w), height: px(34, w)) }.frame(width: px(36, w), height: px(42, w)) }.frame(width: px(156, w), alignment: .trailing) }.frame(height: px(52, w)) }
