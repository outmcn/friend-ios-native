    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack { Image("FriendUserIndex").resizable().scaledToFill().ignoresSafeArea(); Color.black.opacity(0.1).ignoresSafeArea(); ScrollView(showsIndicators:false){ VStack(spacing:0){
                ...
                if let info=model.userInfo { VStack(spacing:0) { profile(info,width:w); stats(info,width:w) }.padding(.horizontal,px(22,w)).padding(.vertical,px(22,w)).background(Image("tabbarBackground").resizable().scaledToFit()).clipShape(RoundedRectangle(cornerRadius:px(28,w))).overlay(RoundedRectangle(cornerRadius:px(28,w)).stroke(Color.white.opacity(0.22),lineWidth:1)); dynamic(info,width:w) } else { Button("请登录"){showLogin=true}.foregroundStyle(.white).padding(.top,100) }
