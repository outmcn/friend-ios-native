...
struct UserCenterView: View {
    ...
    private func userCenterList(width w:CGFloat)->some View { VStack(alignment:.leading,spacing:0){Text("用户中心").font(.system(size:px(32,w),weight:.bold)).foregroundStyle(.white).padding(.bottom,px(40,w));NavigationLink(destination:FollowFansView()){row("FriendUserItemIcon2","关注 / 粉丝",w)};NavigationLink(destination:PrivacyView()){row("FriendUserItemIcon2","隐私设置",w)};row("FriendUserItemIcon1","账户充值",w);Button("退出登录",role:.destructive){Task{await model.logout()}}.padding(.top,22)}.frame(maxWidth:.infinity,alignment:.leading) }
