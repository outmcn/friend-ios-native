...
struct NativeHomeView: View {
    ...
    private func sourceTabBar(width: CGFloat, height: CGFloat, bottomInset: CGFloat) -> some View {
        let barHeight = px(132, width), bottom: CGFloat = 34, frameWidth = width - px(64, width), frameHeight = barHeight - px(16, width)
        return ZStack { RoundedRectangle(cornerRadius: px(22, width)).fill(Color(red: 0.03, green: 0.06, blue: 0.12).opacity(0.98)).frame(width: frameWidth, height: frameHeight); HStack(spacing: 0) { sourceTab(0,"tabbar1White","tabbar1Black","星空",width); sourceTab(1,"tabbar2White","tabbar2Black","发现",width); sourceTab(2,"tabbar3White","tabbar3Black","消息",width); sourceTab(3,"tabbar4White","tabbar4Black","我的",width) }.padding(.horizontal, px(40,width)).frame(width: frameWidth,height: frameHeight) }.frame(width: frameWidth,height: frameHeight).position(x: width/2,y: height-bottom-barHeight/2)
    }
