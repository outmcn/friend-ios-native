...
struct LoginView: View {
    ...
    private func loginField(label: String, text: Binding<String>, secure: Bool) -> some View {
        HStack(spacing: 12) { Text(label).font(.system(size: 15, weight: .bold)).foregroundStyle(Color(red:0.31,green:0.33,blue:0.53)).frame(width: 48, alignment: .leading); if secure { SecureField("请输入", text: text).foregroundStyle(.primary).tint(.primary) } else { TextField("请输入", text: text).foregroundStyle(.primary).tint(.primary).textInputAutocapitalization(.never).autocorrectionDisabled() } }.padding(.horizontal, 18).frame(height: 44).background(Color(.secondarySystemBackground).opacity(0.94)).clipShape(Capsule()).overlay(Capsule().stroke(Color(red:0.31,green:0.33,blue:0.53),lineWidth: 1)).padding(.bottom, 16)
    }
}
