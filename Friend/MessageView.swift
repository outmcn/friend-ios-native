...
struct MessageView: View {
    ...
    private func friendRow(_ friend: FriendItem) -> some View { HStack(spacing: 12) { RemoteAvatar(urlString: friend.headPortrait, size: 54); VStack(alignment: .leading, spacing: 6) { Text(friend.nickName ?? "用户").font(.system(size: 17, weight: .medium)).foregroundStyle(.white); Text("互相关注").font(.caption).foregroundStyle(.white.opacity(0.55)) }; Spacer(); Image(systemName: "chevron.right").foregroundStyle(.white.opacity(0.35)) }.padding(.horizontal, 20).frame(height: 82).overlay(alignment: .bottom) { Color.white.opacity(0.08).frame(height: 1).padding(.leading, 86) } }
}
