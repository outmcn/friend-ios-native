...
struct MessageView: View {
    ...
    private var header: some View { VStack(spacing:0){ HStack{ForEach(["消息","通知","好友"].indices,id:\.self){i in Button{selectedGroup=i}label:{Text(["消息","通知","好友"][i]).font(.system(size:i==selectedGroup ? 18:16,weight:i==selectedGroup ? .bold:.regular)).foregroundStyle(i==selectedGroup ? .white:.white.opacity(0.55)).padding(.horizontal,12).padding(.vertical,10).overlay(alignment:.bottom){if i==selectedGroup{Color(red:0.95,green:0.80,blue:0.38).frame(height:2)}}}};Spacer(minLength:6);HStack{Image(systemName:"magnifyingglass").foregroundStyle(.secondary);TextField(selectedGroup==2 ? "搜索好友" : selectedGroup==1 ? "搜索通知" : "搜索消息",text:$search).foregroundStyle(.primary).tint(.primary)}.padding(.horizontal,8).frame(width:112,height:30).background(Color.white.opacity(0.10)).clipShape(Capsule())}.padding(.horizontal,18).padding(.top,12).padding(.bottom,4)} }
