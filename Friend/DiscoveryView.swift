...
struct DiscoveryPost: Identifiable { let id: Int; let userId: Int?; let name: String; let text: String; let imageURL: String?; let likes: Int; let comments: Int; let favorites: Int; let pushTime: String?; let following: Bool?; let distanceKm: Double?; let headPortrait: String?; init(_ blog: BlogPageResponse) { id=blog.id; userId=blog.userId; name=blog.nickName ?? "用户"; text=blog.content ?? ""; imageURL=blog.images?.first; likes=blog.fabulous ?? 0; comments=blog.comment ?? 0; favorites=blog.favorite ?? 0; pushTime=blog.pushTime; following=blog.following; distanceKm=blog.distanceKm; headPortrait=blog.headPortrait } }
